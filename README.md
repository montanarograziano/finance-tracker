...
- Settings provides self-service account deletion. The database deletes the authenticated user's `auth.users` row and cascades that transaction through every user-owned table; the browser never receives an administrative key.
- Spreadsheet exports neutralize formula injection. GitHub Pages serves no custom response headers, so RLS on Postgres, not HTTP headers, is the authorization boundary; `index.html` runs a two-line framebuster instead, because browsers ignore `frame-ancestors` when it arrives via `<meta>` rather than a real header. That is weaker than the `X-Frame-Options: DENY` Netlify used to send: a sandboxed iframe without `allow-top-navigation` defeats it.
